...
//    private City[] cities;
    ...
    //new Link 直接初始化,就不需要使用setCities了
//    public void setCities(Link cities){
//        this.cities =  cities;
//    }
...
class Link { //链表类，外部能够看见的 只有这一个类
    ...
    private City[] retArray; //增加一个返回的数组属性内容,之所以将其定义为属性,是因为内部类和外部类都可以访问
    ...
    public boolean set(int index, City data){
        if (index >= this.count){
            return false;  //结束方法调用
        }
    ...
    private class Node{  //定义的一个节点
        ...
        //------------根据索引取得数据----------------------------------------------------------------------------------------------------------------------------------------------
             //System.out.println("链表根据索引查询数据：" + link1.get(2));
                /*
                 * 第一次调用getNode函数的是：Link.root等效于【this.root.getNode(2)】;
                 * 1.foot =0 ,                                       /此时的this为：Link.root/
                 ...
                 * 2.foot =1,                                       /此时的this为：Link.root.next/
                 ...
                 * 3.foot=2                                       /此时的this为：Link.root.next.next/
                 ...
                 * 2.this引用传递
                 * 3.next（Node对象引用）的传递
                 * 4.Link.this.foot，内部类访问外部类属性的方式
                 * 5.通过this的引用传递,用foot++来对应this的next变动达到索引的目的
                 *  -----------------------------------------------------------------------------------------------------------------------------------------------------------------
                 */
        public City getNode(int index){
            //Link.this.foot++ == index
            if (Link.this.foot++ == index){  //Link.this.foot++ == index
                return this.data;
        ...
        //------------修改指定索引内容------------------------------------
        public boolean setNode(int index, City data){
...
//        c2.setProvince(pro);
        ...
        City[] cities =  pro.getCities().toArray();
        ...
        Province[] str = new Province[]{new Province(1,"河北省")};
    }
}
